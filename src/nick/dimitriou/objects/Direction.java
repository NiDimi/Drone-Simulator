...
/**
 * Enumerator to set the direction that an object is heading
...
public enum Direction {
   ...
   private static Random random = new Random();
    ...
    public Direction goEast(){
        int num =random.nextInt(3);// get 0,1 or 2
    ...
    public Direction goSouth(){
        return values()[random.nextInt(3)+1];// plus 1 to get either the ordinal value 1,2,3
    }
    ...
    public Direction goWest(){
        return values()[random.nextInt(3)+3];// plus 3 to get either the ordinal value 3,4,5
    }
    ...
    public Direction goNorth(){
        return values()[random.nextInt(3)+5];// plus 5 to get either the ordinal value 5,6,7
    }
    ...
    public Direction add(int num){
        if (ordinal() + num > 7){// if the new value is going out of the higher bound
            return values()[ordinal()+num-8];// subtract the 8
        } else  if (ordinal() + num <0){// if the new value is going out of the lower bound
            return values()[8+ordinal()+num]; // add the 8
        }
        return values()[ordinal()+num];// else just add the number
    }
}
