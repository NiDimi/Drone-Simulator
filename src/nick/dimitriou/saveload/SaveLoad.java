package nick.dimitriou.saveload;
...
/**
 * Class to handle the saving and loading of the application
...
public class SaveLoad {
    ...
    private static File filePicker(){
        FileFilter filter =  new FileFilter() {// adding a filter for only the .ser files
            @Override
            //return only the directories and the .ser files
            public boolean accept(File file) {
                if (file.getAbsolutePath().endsWith(".ser"))// if a file is a .ser file return it
                    return true;
        ...
        JFileChooser chooser = new JFileChooser("C:\\Users\\nickdimi\\IdeaProjects");// use the file chooser to make the user choose a file
    ...
    public static void save(DroneArena arena){
        ...
        try {
            ...
            if(!extension.equals(".ser")){// if it is not .ser added
                fileOut = new FileOutputStream(saveFile+".ser");
            } else {
    ...
    private static void wrongFile(){
        ...
        alert.setContentText("Something went wrong with the file you choose, maybe was the wrong file");
